                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    SelectAddressPanel selectAddressPanel = new SelectAddressPanel(new SelectAddressPanel.SelectAddressListener() {
                        ...
                        public void selectAddress(Address address) {
                            ...
                            if (address != null) {
                                Primer.getMainFrame().getMainFrameUi().getWalletsView().selectWalletPanelByFilename(address.getAddress());
                                Primer.getCoreController().fireDataChangedUpdateNow();
                                closePanel();
                                if (address instanceof HDAccount) {
                                    HDAccountSendPanel hdAccountSendPanel = new HDAccountSendPanel(PrimerjSettings.DONATE_ADDRESS);
                                    hdAccountSendPanel.showPanel();
                                } else if (address.isHDM()) {
                                    SendHDMBitcoinPanel sendHDMBitcoinPanel = new SendHDMBitcoinPanel(PrimerjSettings.DONATE_ADDRESS);
                                    sendHDMBitcoinPanel.showPanel();
                                } else {
                                    if (address.hasPrivKey()) {
                                        SendBitcoinPanel sendBitcoinPanel = new SendBitcoinPanel(PrimerjSettings.DONATE_ADDRESS);
                                        sendBitcoinPanel.showPanel();
                                    } else {
                                        UnSignTxPanel unSignTxPanel = new UnSignTxPanel(PrimerjSettings.DONATE_ADDRESS);
