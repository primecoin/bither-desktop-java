...
import org.primer.viewsystem.listener.ICheckPasswordListener;
...
public class PasswordPanel extends WizardPanel {
    ...
    private void checkValid() {
        ...
        if (passwordLength >= PrimerSetting.PASSWORD_LENGTH_MIN && passwordLength <= PrimerSetting.PASSWORD_LENGTH_MAX) {
            if (etPasswordConfirmIsVisible) {

                int passwordConfirmLength = repeatNewPassword.getPassword().length;
                if (passwordConfirmLength >= PrimerSetting.PASSWORD_LENGTH_MIN && passwordConfirmLength <= PrimerSetting.PASSWORD_LENGTH_MAX) {
                    setOkEnabled(true);
                } else {
                    setOkEnabled(false);
                }
