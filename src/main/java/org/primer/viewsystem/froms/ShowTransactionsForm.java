...
import org.primer.model.TxTableModel;
...
import org.primer.primerjimpl.BlockNotificationCenter;
import org.primer.primerjimpl.TxNotificationCenter;
import org.primer.utils.DateUtils;
import org.primer.utils.LocaliserUtils;
import org.primer.utils.ViewUtil;
import org.primer.viewsystem.action.ShowTransactionDetailsAction;
import org.primer.viewsystem.base.*;
import org.primer.viewsystem.base.renderer.DecimalAlignRenderer;
import org.primer.viewsystem.base.renderer.ImageRenderer;
import org.primer.viewsystem.base.renderer.TrailingJustifiedDateRenderer;
...
public class ShowTransactionsForm implements Viewable, TxNotificationCenter.ITxListener, BlockNotificationCenter.IBlockListener {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                if (Primer.getActionAddress() != null) {
                    ...
                    final List<Tx> actionTxList = Primer.getActionAddress().getTxs();
                    SwingUtilities.invokeLater(new Runnable() {
                        @Override
                        public void run() {
    ...
    private JPanel createTransactionsPanel() {
        ...
        table.setRowHeight(Math.max(PrimerSetting.MINIMUM_ICON_HEIGHT, panelMain.getFontMetrics(FontSizer.INSTANCE.getAdjustedDefaultFont()).getHeight()) + PrimerSetting.HEIGHT_DELTA);
...
//        }
        ...
        tableColumn.setPreferredWidth(statusWidth + PrimerSetting.STATUS_WIDTH_DELTA);
    ...
    private JPanel createTxDetailButtonPanel() {
        ...
        btnAddress = Buttons.newLaunchBrowserButton(new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    String url = BitherUrl.BLOCKCHAIN_INFO_ADDRESS_URL + Primer.getActionAddress().getAddress();
...
//        buttonPanel.add(fill1, constraints);
    ...
    private void scrollPaneSetup() {
        ...
        scrollPane.getHorizontalScrollBar().setUnitIncrement(PrimerSetting.SCROLL_INCREMENT);
        scrollPane.getVerticalScrollBar().setUnitIncrement(PrimerSetting.SCROLL_INCREMENT);
    ...
    public void notificatTx(String address, Tx tx, Tx.TxNotificationType txNotificationType, long deltaBalance) {
        ...
        if (Primer.getActionAddress() != null) {
            actionAddress = Primer.getActionAddress().getAddress();
        }
