...
import org.primer.core.CoreMessageKey;
import org.primer.fonts.AwesomeDecorator;
import org.primer.fonts.AwesomeIcon;
import org.primer.fonts.TitleFontDecorator;
import org.primer.languages.Languages;
import org.primer.languages.MessageKey;
import org.primer.utils.HtmlUtils;
import org.primer.viewsystem.animations.RotatingIcon;
...
public class Labels {
    ...
    public static JLabel newLabel(CoreMessageKey key, Object... values) {
        ...
        JLabel label = new JLabel(Languages.safeText(key.getKey(), values));
    ...
    public static JLabel newTitleLabel(MessageKey key) {
        ...
        // Font
        TitleFontDecorator.apply(label, PrimerUI.BALANCE_HEADER_LARGE_FONT_SIZE);
    ...
    public static JLabel applyitleLabel(JLabel label, MessageKey key) {
    ...
    static JLabel newNoteLabel(CoreMessageKey[] keys, Object[][] values) {
        ...
        for (int i = 0; i < keys.length; i++) {
            if (values.length > 0) {
                ...
                lines[i] = Languages.safeText(keys[i].getKey(), values[i]);
            } else {
                // Key only
                lines[i] = Languages.safeText(keys[i].getKey());
            }
        }
    ...
    public static void decorateStatusLabel(JLabel statusLabel, Optional<Boolean> status) {
        ...
        if (status.isPresent()) {
            if (status.get()) {
                AwesomeDecorator.bindIcon(AwesomeIcon.CHECK, statusLabel, true, PrimerUI.NORMAL_ICON_SIZE);
            } else {
                AwesomeDecorator.bindIcon(AwesomeIcon.TIMES, statusLabel, true, PrimerUI.NORMAL_ICON_SIZE);
            }
        }
    ...
    public static JLabel newIconLabel(AwesomeIcon icon, Optional<MessageKey> key, Object[] values) {
        ...
        AwesomeDecorator.bindIcon(icon, label, false, PrimerUI.NORMAL_ICON_SIZE);
    ...
    public static JLabel newImageLabel(BufferedImage image) {
        ...
        AwesomeDecorator.applyIcon(AwesomeIcon.USER, label, true, PrimerUI.LARGE_ICON_SIZE);
...
//  }
    ...
    public static JLabel newSelectLanguageLabel() {
        ...
        AwesomeDecorator.applyIcon(
                ...
                label,
                true,
                PrimerUI.NORMAL_PLUS_ICON_SIZE
        );
    ...
    public static JLabel newApproximately() {
        ...
        Font font = label.getFont().deriveFont(Font.BOLD, (float) PrimerUI.NORMAL_ICON_SIZE);
        label.setFont(font);
...
//  }
    ...
    public static JLabel newCircle() {
        ...
        AwesomeDecorator.bindIcon(AwesomeIcon.CIRCLE, label, false, PrimerUI.SMALL_ICON_SIZE);
